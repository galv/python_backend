...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        print("GALVEZ: # requests=", len(requests))

        for i, request in enumerate(requests):
            my_input = pb_utils.get_input_tensor_by_name(request, "my_input")
            my_input = np.from_dlpack(my_input)

            corr_id = pb_utils.get_input_tensor_by_name(request, "CORRID")
            corr_id = np.from_dlpack(corr_id).item()

            start = bool(pb_utils.get_input_tensor_by_name(request, "START").as_numpy().item())

            end = bool(pb_utils.get_input_tensor_by_name(request, "END").as_numpy().item())

            out_tensor = pb_utils.Tensor("my_output", my_input)

            response = pb_utils.InferenceResponse(output_tensors=[out_tensor])
            responses.append(response)

            print("GALVEZ:corr_id=", corr_id)
            print("GALVEZ:my_input=", my_input)
